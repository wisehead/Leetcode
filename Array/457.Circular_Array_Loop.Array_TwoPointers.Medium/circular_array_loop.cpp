...
public:
    ...
    bool cal(vector<int>& nums, int start, map<int, int>& mm) {
		...
		int n = nums.size();
        if (!n) return false;
		int i = start, moves = 0, last_steps = 0;
		mm[i] = moves; 
		while (1)
		{
			int steps = nums[i];
            int orig_steps = steps;
			...
			{
                int x = moves - mm[i];
                //cout<<"moves:"<<moves<<"mm[i]"<<mm[i]<<endl;
				if (x > 1)
                {
                    //cout<<"orig_steps:"<<orig_steps<<"  last_steps:"<<last_steps<<endl;
                    while(x > 1)
                    {
